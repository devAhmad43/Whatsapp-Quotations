...
router.post('/signup', async (req, res) => {
    ...
    try {
      ...
      if (existingUser) {
        return res.status(401).json({ message: 'User already exists', userstatus: 0 });
      }
      ...
      newUser.sessionExpiration = new Date().getTime() + (30 * 24 * 60 * 60 * 1000); // 30 days
      // Save the new admin to the database
      await newUser.save();
      // Respond with success
      res.status(200).json({ message: 'User successfully signed up', user: newUser });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Failed to sign up user, try again later' });
    }
  });
